        ...
        <div className="landing-stage justify-center gap-7 py-7 max-sm:justify-start max-sm:gap-5 max-sm:py-5 [@media(max-height:840px)]:justify-start [@media(max-height:840px)]:gap-4 [@media(max-height:840px)]:py-4">
          <div className="mx-auto max-w-[56rem] space-y-4 text-center max-sm:space-y-3 [@media(max-height:840px)]:space-y-3">
            ...
            <p className="mx-auto max-w-[38rem] text-[length:var(--text-body)] leading-[1.5] text-text-secondary [@media(max-height:840px)]:text-[length:var(--text-body-sm)] [@media(max-height:840px)]:leading-[1.4]">
              Mnemos Mundi combina experiência, persistência e descoberta em uma única infraestrutura educacional.
            </p>
          </div>
          ...
          <div className="mx-auto flex w-full max-w-[44rem] flex-col items-center gap-3 sm:flex-row sm:justify-center [@media(max-height:840px)]:gap-2.5">
            <Link
              href="/mnemos-mundi"
              className="inline-flex min-h-13 w-full items-center justify-center rounded-full border border-[#5bc0eb] bg-[#5bc0eb14] px-6 text-[length:var(--text-body)] font-semibold text-text-primary sm:min-h-14 sm:w-auto sm:min-w-[16rem]"
            >
              Explorar protótipo
            </Link>
            <a
              href="#para-instituicoes"
              className="inline-flex min-h-13 w-full items-center justify-center rounded-full border border-white/12 bg-white/5 px-6 text-[length:var(--text-body)] font-medium text-text-secondary sm:min-h-14 sm:w-auto sm:min-w-[16rem]"
            >
          ...
          <div className="mx-auto flex w-full max-w-[46rem] flex-col items-center gap-3 border-t border-white/8 pt-4 text-center [@media(max-height:840px)]:gap-2.5 [@media(max-height:840px)]:pt-3">
            ...
            <ul className="flex flex-wrap items-center justify-center gap-2 text-[length:var(--text-caption)] text-text-secondary sm:text-[length:var(--text-body-sm)]">
              {stack.map((item) => (
                <li
                  key={item}
                  className="rounded-full border border-white/8 bg-white/[0.03] px-3 py-1.5 sm:px-3.5 sm:py-2"
                >
                  {item}
                </li>
              ))}
